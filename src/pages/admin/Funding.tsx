...
interface Applicant { id: string; name: string; organization: string; email: string; balance: number }
export default function AdminFunding() {
  ...
  async function load() {
    if (!supabase) return;
    const [withdrawals, profiles, wallets] = await Promise.all([
      supabase.from("withdrawal_requests").select("*").order("created_at", { ascending: false }),
      supabase.from("profiles").select("id,name,organization,email").order("name"),
      supabase.from("wallets").select("user_id,balance"),
    ]);
    if (!withdrawals.error) setRequests(withdrawals.data ?? []);
    if (!profiles.error) {
      const balances = new Map((wallets.data ?? []).map(wallet => [wallet.user_id, Number(wallet.balance ?? 0)]));
      setApplicants((profiles.data ?? []).map(profile => ({ ...profile, balance: balances.get(profile.id) ?? 0 })));
    }
  }
  async function adjust(e: FormEvent) {
    ...
    if (error) setMessage(error.message); else { setMessage("Balance updated."); setAdjustment(""); setMemo(""); await load(); }
  }
  ...
  return <div className="space-y-6">
    ...
    <form onSubmit={adjust} className="card p-5 grid gap-4 md:grid-cols-4">
      <select className="input" value={userId} onChange={e => setUserId(e.target.value)} required>
        ...
        {applicants.map(applicant => <option key={applicant.id} value={applicant.id}>{applicant.name || applicant.email || "Unnamed user"} · {applicant.email} · {formatCurrency(applicant.balance)}</option>)}
      </select>
    ...
    <div className="card overflow-x-auto"><div className="p-4 font-semibold">Registered users</div><table className="w-full text-sm min-w-[640px]"><thead className="bg-ink-50 dark:bg-ink-900 text-xs uppercase muted"><tr><th className="text-left p-3">User</th><th className="text-left p-3">Organization</th><th className="text-left p-3">Current balance</th><th className="text-left p-3">Funding action</th></tr></thead><tbody className="divide-y divide-ink-100 dark:divide-ink-800">{applicants.map(applicant => <tr key={applicant.id}><td className="p-3"><div className="font-medium">{applicant.name || "Unnamed user"}</div><div className="text-xs muted">{applicant.email}</div></td><td className="p-3">{applicant.organization || "—"}</td><td className="p-3 font-medium">{formatCurrency(applicant.balance)}</td><td className="p-3"><button className="btn-outline py-1.5" onClick={() => setUserId(applicant.id)}>Select user</button></td></tr>)}</tbody></table>{applicants.length === 0 && <div className="p-5 text-sm muted">No registered users found. Run the latest Supabase schema migration.</div>}</div>
