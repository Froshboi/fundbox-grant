...
interface Withdrawal { id: string; user_id: string; amount: number; status: string; payout_account_last4: string; created_at: string }
export default function AdminFunding() {
  ...
  async function load() { if (!supabase) return; const { data } = await supabase.from("withdrawal_requests").select("*").order("created_at", { ascending: false }); setRequests(data ?? []); }
  ...
  return <div className="space-y-6">
    ...
    <div className="card overflow-x-auto"><table className="w-full text-sm min-w-[700px]"><thead className="bg-ink-50 dark:bg-ink-900 text-xs uppercase muted"><tr><th className="text-left p-3">User</th><th className="text-left p-3">Amount</th><th className="text-left p-3">Payout account</th><th className="text-left p-3">Created</th><th className="text-left p-3">Action</th></tr></thead><tbody className="divide-y divide-ink-100 dark:divide-ink-800">{requests.map(request => <tr key={request.id}><td className="p-3 font-mono text-xs">{request.user_id}</td><td className="p-3">{formatCurrency(request.amount)}</td><td className="p-3">••••{request.payout_account_last4}</td><td className="p-3 text-xs muted">{formatDate(request.created_at)}</td><td className="p-3">{request.status === "Pending" ? <div className="flex gap-2"><button className="btn-primary py-1" onClick={() => void review(request.id, "Approved")}>Approve</button><button className="btn-outline py-1" onClick={() => void review(request.id, "Rejected")}>Reject</button></div> : <span className="chip bg-ink-100 dark:bg-ink-800">{request.status}</span>}</td></tr>)}</tbody></table></div>
  </div>;
}
