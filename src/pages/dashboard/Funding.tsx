...
export default function Funding() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("payout_accounts").select("bank_name,account_last4").eq("user_id", user.id).maybeSingle(),
    ]).then(([wallet, tx, requests, payout]) => {
      setBalance(Number(wallet.data?.balance ?? 0));
      setTransactions(tx.data ?? []);
      setWithdrawals(requests.data ?? []);
      if (payout.data) { setBankName(payout.data.bank_name ?? ""); setLast4(payout.data.account_last4 ?? ""); }
    });
  }, [user]);
  ...
  async function submit(e: FormEvent) {
    ...
    if (!/^\d{4}$/.test(last4)) { setMessage("Enter the last four digits of the approved payout account."); return; }
    const value = Number(amount);
    if (!value || value <= 0 || value > balance) { setMessage("Enter an amount no greater than your available balance."); return; }
    const payout = await supabase.from("payout_accounts").upsert({ user_id: user.id, bank_name: bankName, account_last4: last4, status: "Pending" });
    if (payout.error) { setMessage(payout.error.message); return; }
    const request = await supabase.from("withdrawal_requests").insert({ user_id: user.id, amount: value, payout_account_last4: last4 }).select().single();
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <form onSubmit={submit} className="card p-5 space-y-4">
        ...
        <div><label className="label">Bank or payout provider</label><input className="input" value={bankName} onChange={e => setBankName(e.target.value)} required /></div>
        <div><label className="label">Account last four digits</label><input className="input" inputMode="numeric" maxLength={4} value={last4} onChange={e => setLast4(e.target.value.replace(/\D/g, ""))} required /></div>
      ...
      <div className="card p-5"><div className="font-semibold">Withdrawal requests</div><div className="mt-3 divide-y divide-ink-100 dark:divide-ink-800">{withdrawals.map(item => <div key={item.id} className="py-3 flex justify-between text-sm"><span>{formatCurrency(item.amount)} · account ending {item.payout_account_last4}</span><span className="chip bg-ink-100 dark:bg-ink-800">{item.status}</span></div>)}{withdrawals.length === 0 && <div className="py-6 text-sm muted">No withdrawal requests.</div>}</div></div>
    </div>
